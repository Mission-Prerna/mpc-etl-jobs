...
api.config['CELERY_BROKER_URL'] = 'amqp://testing:blody_mary_789@165.232.182.146:5672'
api.config['CELERY_RESULT_BACKEND'] = 'rpc://testing:blody_mary_789@165.232.182.146:5672'
...
@celery.task
def add_task_volunteer(data):
    try:
        connection = get_connection()
        cursor = connection.cursor()
        for form_response in data:
...
def save_volunteer():
    ...
    if not request_json["data"]:
        ...
        add_task_volunteer.delay(request_json["data"])
...
def save_e_pathshala_quiz_data():
    ...
    if not request_json["data"]:
        ...
        add_task_pathshala.delay(request_json["data"])
